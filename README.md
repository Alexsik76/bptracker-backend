...
| `POST` | `/api/v1/auth/passkey/register/begin` | Ініціює реєстрацію Passkey (потребує авторизації) |
| `POST` | `/api/v1/auth/passkey/register/complete` | Завершує реєстрацію Passkey (потребує авторизації) |
...
| `POST` | `/api/v1/auth/dev-login` | Швидкий вхід під дев-користувачем (лише в `Development` режимі, див. [README_DEV.md](../README_DEV.md)) |
...
### Schemas (потребує авторизації)
| Метод | URL | Опис |
|---|---|---|
| `GET` | `/api/v1/schemas/active` | Активна схема лікування поточного користувача |
| `GET` | `/api/v1/schemas` | Список усіх схем користувача |
| `POST` | `/api/v1/schemas` | Створити схему `{doctor, prescribedOn?, schedule, setActive}` |
| `PUT` | `/api/v1/schemas/{id}` | Оновити схему |
| `POST` | `/api/v1/schemas/{id}/activate` | Зробити схему активною |

Активною може бути максимум одна схема для кожного користувача; `setActive` при створенні та `activate` атомарно (в транзакції) знімають прапорець з попередньої активної.

### Reminders (потребує авторизації)
| Метод | URL | Опис |
|---|---|---|
| `POST` | `/api/v1/reminders/template` | Створити або перезаписати шаблон розкладу прийому (ініціює генерацію прийомів на сьогодні/майбутнє) |
| `PATCH` | `/api/v1/reminders/template/{id}` | Частково оновити шаблон розкладу за його ID |
| `GET` | `/api/v1/reminders/template/active` | Отримати поточний активний шаблон розкладу прийому ліків |
| `POST` | `/api/v1/reminders/confirm` | Підтвердити прийом ліків для певного періоду (`Morning`, `Day`, `Evening`) із таймзоною клієнта. Тіло: `{period, timezone}`. |
| `GET` | `/api/v1/reminders/today` | Отримати розклад прийомів ліків на сьогодні з таймзоною клієнта (`?timezone=Europe/Kyiv`) та їх статусами. |
| `GET` | `/api/v1/reminders/reports` | Отримати звіт про виконання прийомів ліків за останні N днів (`?days=30`). |

### Web Push (потребує авторизації)
| Метод | URL | Опис |
|---|---|---|
| `GET` | `/api/v1/push/vapid-public-key` | Отримати публічний ключ VAPID (публічний ендпоінт) |
| `POST` | `/api/v1/push/subscribe` | Підписатися на push-повідомлення (надсилається об'єкт підписки браузера) |
| `POST` | `/api/v1/push/unsubscribe` | Скасувати підписку на push-повідомлення |
| `POST` | `/api/v1/push/test` | Надіслати тестове push-повідомлення поточному користувачу |
...
- **Безпечна реєстрація Passkey:** реєстрація Passkey вимагає активної сесії (`RequireAuthorization`). Прив'язка нового ключа здійснюється виключно до ідентифікатора користувача з поточної сесії, ігноруючи будь-який email з тіла запиту (запобігання account takeover).
- **Список дозволених адрес (Email Allowlist):** вхід/реєстрація обмежені списком дозволених адрес у змінній `ALLOWED_EMAILS`. Перевірка здійснюється у `magic-link/consume` без розкриття інформації про статус перевірки (повертається стандартна успішна відповідь без створення користувача чи сесії для запобігання перебору email-адрес).
...
> 💡 **Рекомендований спосіб розробки**: Дивіться [README_DEV.md](../README_DEV.md) у корені проекту для запуску повного локального середовища розробки (включаючи бекенд, базу даних з авто-сідером та проксі-налаштування для фронтенду).
...
- **Database:** використовується `Npgsql.EntityFrameworkCore.PostgreSQL`. `TreatmentSchema.Id` — `Guid`. Поля: `Doctor` (text), `PrescribedOn` (date), `CreatedAt` (timestamptz, default now()), `IsActive` (bool), `ScheduleDocument` (`JsonDocument?`, jsonb), `UserId` (uuid, FK на `Users`). Формат `ScheduleDocument`: обʼєкт із ключами `Morning`/`Day`/`Evening`, кожен — масив `{Amount, Medicine, Condition}`, усі значення рядки (зокрема `Amount` — рядок, напр. `"0.5"`). У БД ключі канонічний PascalCase; вхід API приймає їх case-insensitive і нормалізує. Усі схеми та вимірювання ізольовані на рівні користувачів через `UserId`, а часові поля (зокрема `CreatedAt` у схемах та `RecordedAt` у замірах) використовують тип `timestamptz` у БД та `DateTimeOffset` у C#.

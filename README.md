...
├── Program.cs          Startup, DI, CORS, Rate Limiting, Serilog, автоміграції
...
Покриває: auth (magic link, session), measurements (CRUD, IDOR, with-photo), export (outbox, rate limit), health, photo-api service (upload, recognize: local success, fallback на Gemini, обидва fail, disabled short-circuit).

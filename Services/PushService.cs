...
public class PushService : IPushService
{
    ...
    public async Task<(int sent, int failed, int subscriptions)> SendCustomToUserAsync(
        Guid userId,
        string title,
        string body,
        ...
        bool? renotify = null)
    {
        ...
        foreach (var sub in subscriptions)
        {
            ...
            if (ttl.HasValue)
            {
                message.TimeToLive = ttl.Value;
            }

            _logger.LogInformation("Sending custom push notification to {Endpoint} with Urgency: {Urgency}, TTL: {TTL}", 
                sub.Endpoint, urgency, ttl.HasValue ? ttl.Value.ToString() : "default");
        ...
        return (sent, failed, subscriptions.Count);
    }
}
